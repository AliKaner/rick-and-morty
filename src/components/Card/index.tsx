...
export function Card({ cardProps }: { cardProps: CharacterType }) {
  ...
  return (
    <div className="flex flex-col md:flex-row bg-primary text-white rounded-xl h-144 w-full xs:w-144 items-center justify-center gap-4 p-4">
      <img
        className="w-full md:w-1/3 rounded-xl md:rounded-l-xl"
        src={cardProps.image}
      />
      <div className="md:w-2/3 md:ml-4 flex flex-col gap-4 justify-around text-left">
        <div className="text-left flex  flex-col ">
          <h1 onClick={onCardClickHandle} className="cursor-pointer hover:text-green-300 text-2xl font-extrabold">{cardProps.name}</h1>
        ...
        <div className="cursor-pointer hover:text-green-700" onClick={onLocationClickHandle}>
          <div>Last known location:</div>
          <div>{cardProps.location.name}</div>
        </div>
        <div className="cursor-pointer hover:text-green-500" onClick={onEpisodeClickHandle}>
